...
func (s *Server) Stats(c *gin.Context) {
	// Count unique physical keys (by key_hash), not DB records
	// Same key in multiple groups should count as 1
	var activeKeys, invalidKeys int64
	s.DB.Model(&models.APIKey{}).
		Where("status = ? AND key_hash IS NOT NULL AND key_hash != ''", models.KeyStatusActive).
		Select("COUNT(DISTINCT key_hash)").Scan(&activeKeys)
	s.DB.Model(&models.APIKey{}).
		Where("status = ? AND key_hash IS NOT NULL AND key_hash != ''", models.KeyStatusInvalid).
		Select("COUNT(DISTINCT key_hash)").Scan(&invalidKeys)
