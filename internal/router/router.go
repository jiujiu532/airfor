...
func registerProtectedAPIRoutes(api *gin.RouterGroup, serverHandler *handler.Server) {
	...
	pool := api.Group("/pool")
	{
		pool.GET("/stats", serverHandler.GetPoolStats)
		pool.GET("/config", serverHandler.GetPoolConfig)
		pool.PUT("/config", serverHandler.UpdatePoolConfig)
		pool.POST("/register/start", serverHandler.StartRegistration)
		pool.POST("/register/stop", serverHandler.StopRegistration)
		pool.GET("/register/stats", serverHandler.GetRegistrationStats)
		pool.GET("/register/logs", serverHandler.GetRegistrationLogs)
	}
}
