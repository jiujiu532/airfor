...
func NewApp(params AppParams) *App {
	return &App{
		...
		poolService:       params.PoolService,
...
func (a *App) Start() error {
	...
	// Wire up pool reload callbacks for async key operations
	if a.poolService != nil {
		reloadFn := func() { a.poolService.ReloadKeys() }
		if a.keyImportService != nil {
			a.keyImportService.OnKeysChanged = reloadFn
		}
		if a.keyDeleteService != nil {
			a.keyDeleteService.OnKeysChanged = reloadFn
		}
	}
	...
	if a.configManager.IsMaster() {
		...
		// 初始化速率限制池
		if err := a.poolService.Initialize(); err != nil {
			logrus.WithError(err).Warn("Failed to initialize rate-limited pool, will use standard key rotation")
		}
	} else {
