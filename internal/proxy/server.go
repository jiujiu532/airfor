...
type ProxyServer struct {
	...
	poolService       *ratelimitpool.PoolService
...
func (ps *ProxyServer) executeRequestWithRetry(
	...
	// Select key: use rate-limited pool if enabled, otherwise standard rotation
	var apiKey *models.APIKey
	var usedRateLimitPool bool

	if ps.poolService != nil && ps.poolService.IsEnabled() {
		keyValue, keyID, err := ps.poolService.AcquireKey(group.ID)
		if err != nil {
			logrus.Errorf("[RateLimitPool] Failed to acquire key for group %s on attempt %d: %v", group.Name, retryCount+1, err)
		...
		usedRateLimitPool = true
	} else {
		var selectErr error
		apiKey, selectErr = ps.keyProvider.SelectKey(group.ID)
		if selectErr != nil {
			logrus.Errorf("Failed to select a key for group %s on attempt %d: %v", group.Name, retryCount+1, selectErr)
			response.Error(c, app_errors.NewAPIError(app_errors.ErrNoKeysAvailable, selectErr.Error()))
			ps.logRequest(c, originalGroup, group, nil, startTime, http.StatusServiceUnavailable, selectErr, isStream, "", channelHandler, bodyBytes, models.RequestTypeFinal)
			return
		}
	}
	...
	if err != nil || (resp != nil && resp.StatusCode >= 400 && resp.StatusCode != http.StatusNotFound) {
		...
		// 使用解析后的错误信息更新密钥状态
		if usedRateLimitPool {
			ps.poolService.ReportKeyResult(apiKey.ID, false, statusCode)
		} else {
