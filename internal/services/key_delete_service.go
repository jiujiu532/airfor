...
type KeyDeleteService struct {
	...
	OnKeysChanged func() // called after delete completes to sync pool
}
...
func (s *KeyDeleteService) runDelete(group *models.Group, keys []string) {
	// Compute hashes before deleting (for cross-group sync)
	var keyHashes []string
	for _, k := range keys {
		keyHashes = append(keyHashes, s.KeyService.EncryptionSvc.Hash(strings.TrimSpace(k)))
	}
	...
	if deletedCount > 0 {
		s.KeyService.syncDeleteKeysByHash(group.ID, keyHashes)
	}
